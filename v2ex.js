/**
 * V2EX 每日签到 (Surge / Quantumult X / Loon)
 * cron: 0 9 * * *
 * const $ = new Env('V2EX 签到');
 */
...
if (!cookieVal) {
  $.msg($.name, "未捕获到 Cookie ❗️", "请先在浏览器登录并抓取");
  $.done();
}
...
function request(opt) {
  return new Promise((resolve, reject) => {
    ...
    fn(opt, (err, resp, data) => (err ? reject(err) : resolve({ resp, data })));
  });
}
...
(async () => {
  try {
    ...
    const m = html.match(/\/mission\/daily\/redeem\?once=([^"&']+)/);
    if (!m) throw new Error("未匹配到 once 参数");

    $.log("🔔 触发签到");
    const { data: result } = await request({
      url: `https://www.v2ex.com/mission/daily/redeem?once=${encodeURIComponent(
        m[1]
      )}`,
      headers: baseHeaders,
    });
